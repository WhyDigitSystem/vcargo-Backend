...
public interface TvehicleRepo extends JpaRepository<TvehicleVO, Long>{
    ...
    @Query(
        value =
            "SELECT t.* FROM tvehicle t " +
            "WHERE (:branchCode IS NULL OR :branchCode = '' OR t.branchcode = :branchCode) And t.userid = :userId " +
            "AND ( " +
    ...
    Page<TvehicleVO> getTvehiclesByOrgId(
